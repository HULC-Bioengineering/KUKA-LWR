...
COLORS = [itertools.cycle(['#28ABE3', '#236CF5']),
          itertools.cycle(['#32CB2E', '#008000']),
          itertools.cycle(['#ff0000', '#660000']),
...
if __name__=="__main__":
    ...
    spatial_path_file = BASE_FILEPATH + 'paths\\load\\x_10_n_cropped_at_5.csv'
    log_file = BASE_FILEPATH + 'experiments\\x_10_n_v1_cropped_at_5.csv'
    ...
    fx_fy_fz = setup_plots((0, 0), 3, 4, 3, 'Fx', 'Fy', 'Fz')
    fx_t = setup_plots((4, 0), 3, 1, 2, 't', 'Fx')
    fy_t = setup_plots((5, 0), 3, 1, 2, 't', 'Fy')
    fz_t = setup_plots((6, 0), 3, 1, 2, 't', 'Fz')
    
    #pidx_t = setup_plots((6, 0), 3, 1, 2, 'T', 'PIDx')
    #pidy_t = setup_plots((7, 0), 3, 1, 2, 'T', 'PIDy')
    #pidz_t = setup_plots((8, 0), 3, 1, 2, 'T', 'PIDz')
    
    #px_py_pz = setup_plots((8, 0), 3, 4, 3, 'Px', 'Py', 'Pz')
    #px_t = setup_plots((12, 0), 3, 1, 2, 'T', 'Px')
    #py_t = setup_plots((13, 0), 3, 1, 2, 'T', 'Py')
    #pz_t = setup_plots((14, 0), 3, 1, 2, 'T', 'Pz')
    
    blue_patch = mpatches.Patch(color='#28ABE3', label='Measured')
    black_patch = mpatches.Patch(color='#696969', label='Desired')
    l = fx_fy_fz.legend(handles=[blue_patch, black_patch])
    l.draggable()
    ...
    #plt.show()
    fig.savefig(RESULTS_PATH + log_filename[:-4] + '2.pdf', dpi=150)
    #plt.close()
