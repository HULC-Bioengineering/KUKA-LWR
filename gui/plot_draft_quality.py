...
#    log_file = BASE_FILEPATH + 'results\\poke\\move\\loop_10_y_15_plat_move.csv'
    spatial_path_file = BASE_FILEPATH + 'paths\\load\\thick_y2.csv'
    log_file = BASE_FILEPATH + 'experiments\\thick_y.csv'
    ...
    fx_t = setup_plots((0, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$F_X [N]$')
    fy_t = setup_plots((1, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$F_Y [N]$')
    fz_t = setup_plots((2, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$F_Z [N]$')
    f_euc = setup_plots((3, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$F_{EUC} [N]$')
    px_t = setup_plots((4, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$P_X [mm]$')
    py_t = setup_plots((5, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$P_Y [mm]$')
    pz_t = setup_plots((6, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$P_Z [mm]$')
    p_euc = setup_plots((7, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$P_{EUC} [mm]$')
    pidx_t = setup_plots((8, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$PID_X [mm]$')
    pidy_t = setup_plots((9, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$PID_Y [mm]$')
    pidz_t = setup_plots((10, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$PID_Z [mm]$')
    lwrx_t = setup_plots((11, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$LWR_x [N]$')
    lwry_t = setup_plots((12, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$LWR_y [N]$')
    lwrz_t = setup_plots((13, 0), 3, 1, 2, [0,t[-1] + 0.1], '$time [s]$', '$LWR_z [N]$')
    ...
    #plt.show()
    fig.savefig(RESULTS_PATH + log_filename[:-4] + '_draft.pdf', dpi=150,  bbox_inches='tight')
    #plt.close()
