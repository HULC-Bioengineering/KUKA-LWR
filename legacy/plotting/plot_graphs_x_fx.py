...
def setup_plots(placement, columns, rows, dimension, x_label='', y_label='', z_label=''):
    if dimension == 3:
...
if __name__=="__main__":
    ...
    fx_t = setup_plots((4, 0), 3, 1, 2, '$t [s]$', '$F_X [mm]$')
    fy_t = setup_plots((5, 0), 3, 1, 2, '$t [s]$', '$F_Y [mm]$')
    fz_t = setup_plots((6, 0), 3, 1, 2, '$t [s]$', '$F_Z [mm]$')
    
    fz_t.yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    ...
    #px_py_pz = setup_plots((0, 0), 3, 4, 3, '$P_X [mm]$', '$P_Y [mm]$', '$P_Z [mm]$')
    #px_t = setup_plots((4, 0), 3, 1, 2, 'T', '$P_X [mm]$')
    #py_t = setup_plots((5, 0), 3, 1, 2, 'T', '$P_Y [mm]$')
    #pz_t = setup_plots((6, 0), 3, 1, 2, 'T', '$P_Z [mm]$')
    #ff0000', '#660000
    ...
    # y '#32CB2E', '#008000'
    #blue_patch = mpatches.Patch(color='#008000', label='$P_{msr}$')
    #black_patch = mpatches.Patch(color='#696969', label='$P_{dsr}$')
    #l = px_py_pz.legend(handles=[blue_patch, black_patch])
    #force_patch = mpatches.Patch(color='#008000', label='$F_{msr}$')
    #force_patch2 = mpatches.Patch(color='#696969', label='$F_{dsr}$')   
    #l = fx_fy_fz.legend(handles=[force_patch, force_patch2]) 

    # z 28ABE3
    #blue_patch = mpatches.Patch(color='#32CB2E', label='$P_{msr}$')
    #black_patch = mpatches.Patch(color='#696969', label='$P_{dsr}$')
    #l = px_py_pz.legend(handles=[blue_patch, black_patch])
    ...
    #blue_patch = mpatches.Patch(color='#28ABE3', label='$Fz_{msr}$')
    #red_patch = mpatches.Patch(color='#ff0000', label='$Fy_{msr}$')
    #green_patch = mpatches.Patch(color='#32CB2E', label='$Fx_{msr}$')
    #black_patch = mpatches.Patch(color='#696969', label='$F_{dsr}$')
    #l = fx_fy_fz.legend(loc='upper left', handles=[green_patch, red_patch, blue_patch, black_patch])
    ...
    #plt.show()
    fig.savefig(RESULTS_PATH + log_filename[:-4] + '_force.pdf', dpi=150)
    #plt.close()
