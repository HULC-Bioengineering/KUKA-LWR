...
if __name__=="__main__":
    ...
    spatial_path_file = BASE_FILEPATH + 'paths\\load\\s.csv'
    log_file = BASE_FILEPATH + 'experiments\\s_1_cutting_48.csv'
    ...
    pidx_t = setup_plots((0, 0), 3, 1, 2, '$t [s]$', '$PID_x [mm]$')
    pidy_t = setup_plots((1, 0), 3, 1, 2, '$t [s]$', '$PID_y [mm]$')
    pidz_t = setup_plots((2, 0), 3, 1, 2, '$t [s]$', '$PID_z [mm]$')
    
    #pidy_t.set_ylim([0.45, 0.55])
    ...
    for start, end in zip(ps[:-1], ps[1:]):
        ...
        pid_xyz.plot(pi_x, pi_y, pi_z, color=c)
    ...
    #plt.show()
    fig.savefig(RESULTS_PATH + log_filename[:-4] + '_pid.pdf', dpi=150)
    #plt.close()
